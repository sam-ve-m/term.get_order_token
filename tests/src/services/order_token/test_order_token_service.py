...
from tests.src.services.order_token.stubs import payload_validated, stub_unique_id
...
async def test_when_sign_term_and_generate_token_with_successfully_then_return_order_token(mock_term_sign, mock_generate_token):
    result = await OrderToken.sign_term_and_generate_order_token(payload_validated=payload_validated, jwt="123", unique_id=stub_unique_id)
